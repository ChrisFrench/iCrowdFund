			...
			else
			{
			    ...
			    var a = new Request({
			        ...
			        onSuccess: function(response){
						SqueezeBox.open(com_tienda.jbase + "index.php?option=com_tienda&view=carts&tmpl=component&funding=1", { handler : 'iframe', size : {x: (w.width() - 100) , y: (w.height() - 100) }, ,onClose:function(){var js =window.location.reload();} } );
			        }
			    }).send();
...
<div>
    ...
    <div id='add_to_cart_<?php echo $item->product_id; ?>' class="add_to_cart" style="display: block;"> 
        ...
        <?php $onclick = "Dsc.formValidation( '".JRoute::_( @$this->validation )."', 'validationmessage_".$item->product_id."', 'addtocart', document.".$formName.", true, '".JText::_('COM_TIENDA_VALIDATING')."', Dsc.displayLightboxLevels );"; ?>
        ...
        if (empty($item->product_check_inventory) || (!empty($item->product_check_inventory) && empty($this->invalidQuantity)) ) :
            switch (Tienda::getInstance()->get('cartbutton', 'image')) 
            {
                case "button":
                    ?>
                    <input onclick="<?php echo $onclick; ?>" value="Fund <?php  echo TiendaHelperProduct::dispayPriceWithTax($item->price, $item->tax, $this->show_tax); ?>
" type="button" class="btn btn-funding " />
                    <?php
                    break;
                case "image":
                default:
