...
class plgTiendaPayment_wepay extends TiendaPaymentPlugin
{
    ...
    {    
        ...
        if($found) {
            ...
            $item->campaign = TiendaHelperCampaign::getCampaignFromProduct($item->product_id);



            //THIS LOGIC IS BACKWARDS, Instead of checking  for what this funding level is we need to check against the ones it is not
                if($item->campaign->fundingtype == 1 || $item->campaign->fundingtype == 3 || $item->campaign->fundingtype == 4 ) {
                   $found = false; 
                }
            }
        }
        ...
        */
        ...
        if ( $object->error == false ) {
           ...
           /* $order = unserialize( $session->get( 'tienda.opc.order' ) );
            ...
            $wepay_cc_info['expiration_year'] = $data['cardexp_year'];
            $address = array(
                'address1'  => $BillingAddress->address_1. " ".$BillingAddress->address_2,
                'city'      => $BillingAddress->city,
                'state'     => $this->getTwoLetterStateCode($BillingAddress->zone_id),
                'country'   => $this->getTwoLetterCountryCode($BillingAddress->country_id),
                'zip'       => $BillingAddress->postal_code
            ); 
            FB::log($wepay_cc_info);
    ...
    function _process() {
        ...
        FB::log($wepay_checkout_data);
        $wepay_checkout = $this->_wepayCheckout( $wepay_checkout_data, $wepay );
        if(empty($wepay_checkout->checkout_id)) {
    ...
    function _wepayCheckout( $wepay_checkout_data, $wepay ) {
        ...
        if ( empty( $error ) &&  $checkout->checkout_id ) {
