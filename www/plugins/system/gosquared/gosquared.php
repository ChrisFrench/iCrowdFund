 ...
 * @author		Aaron Parker <aaron@gosquared.com> and Geoff Wagstaff <geoff@gosquared.com>
...
class plgSystemGosquared extends JPlugin {
	...
	function onAfterRender() {
		...
		if($acct !='' && $acct != 'GSN-000000-X' && preg_match('/GSN-[0-9]{6,7}-[A-Z]{1}/', $acct)) {
			...
			if(!$runAdmin) {
				if(version_compare(JVERSION, '1.6.0', 'ge')) {  // If the version is >= 1.7 do this
					$app = JFactory::getApplication();
					if($app->isAdmin()) {
						return; //This is an admin page so don't run
					}
				} else {  // It's an older version so do this
					global $mainframe;
