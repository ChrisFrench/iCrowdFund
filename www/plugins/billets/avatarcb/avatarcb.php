 ...
 *
 */
class plgBilletsAvatarcb extends BilletsPluginBase {
	...
	function _isInstalled() {
		...
		$db -> setQuery(str_replace('#__', $prefix, "SHOW TABLES LIKE '#__comprofiler'"));
	...
	function onBeforeDisplayCommentAuthorImage($comment, $data, $user) {
		...
		if ($avatar = plgBilletsAvatarcb::getUserAvatar($comment -> userid_from)) {
			$file = JPATH_SITE . DS . 'images' . DS . 'comprofiler' . DS . $avatar;
			if (file_exists($file)) {
				$comment -> authorimage = "<img src='" . JURI::root() . "/images/comprofiler/" . $avatar . "' style='max-width:48px;'>";
			}
		}
