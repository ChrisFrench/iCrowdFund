...
class AmbraFile extends JObject 
{
	/**
	...
	function setDirectory( $dir=null ) 
	{
		...
		$helper = AmbraHelperBase::getInstance();
	...
	function getDirectory( $media='images' )
	{
		...
		{
			$this->_directory = Ambra::getPath( $media );
		}
		return $this->_directory;
	}
	...
	{
		$success = false;
		$config = &AmbraConfig::getInstance();
	...
	function getArrayListStorageMethods() 
	{
		...
		if (!is_array($instance)) {
			$instance = array();
			$data = &AmbraFile::getStorageMethods();
