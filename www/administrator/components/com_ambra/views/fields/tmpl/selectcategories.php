...
<h1 style="margin-left: 2%; margin-top: 2%;"><?php echo JText::_( "Select Categories for" ); ?>: <?php echo $row->field_name; ?></h1>
        ...
        <?php foreach (@$items as $item) : ?>
            <tr class='row<?php echo $k; ?>'>
				...
				<td style="text-align: center;">
					...
					<?php
                    $keynames = array();
                    $keynames['field_id'] = $row->field_id;
                    $keynames['category_id'] = $item->category_id;
					?>
					<?php $table->load( $keynames ); ?>
					<?php echo AmbraGrid::enable(!empty($table->field_id), $i, 'selected.'); ?>
				</td>
                <td style="text-align: center;">
                    <?php echo AmbraGrid::enable(!empty($table->required), $i, 'required.'); ?>
                </td>
			</tr>
			<?php $i=$i+1; $k = (1 - $k); ?>
			<?php endforeach; ?>
