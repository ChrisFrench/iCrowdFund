 ...
 * @since		1.5
 */
class TiendaModelElementcampaign extends DSCModelElement
{
	var $_list = null;
	
	var $_page = null;
	
	var $select_title_constant = 'Select Campaign';
	...
	function getList( $refresh = false )
	{
		...
		// Keyword filter
		if ($search) {
			$where[] = 'LOWER( tbl.campaign_id ) LIKE '.$db->Quote( '%'.$db->getEscaped( $search, true ).'%', false );
			$where[] = 'LOWER( tbl.campaign_name ) LIKE '.$db->Quote( '%'.$db->getEscaped( $search, true ).'%', false );
		}
		// Build the where clause of the query
		$where = (count($where) ? ' WHERE '.implode(' OR ', $where) : '');

		// Get the total number of records
		$query = 'SELECT COUNT(c.campaign_id)' .
				' FROM #__tienda_campaigns AS tbl' .
				$where;
		$db->setQuery($query);
		$total = $db->loadResult();
