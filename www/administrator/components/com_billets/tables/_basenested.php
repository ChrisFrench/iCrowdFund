 ...
 * 
 * @version		$Id: table.php 12779 2009-09-18 12:16:15Z eddieajau $
...
class BilletsTableNested extends BilletsTable
{
	/**
	 * Extends to add some fields based on calculations
	 * @see billets/admin/tables/BilletsTable#load($oid, $key)
	 */
	...
	function delete( $id=null ) 
	{
		...
		if (empty($this->$key))
		{
			$this->setError( JText::_('COM_BILLETS_INVALID_ITEM') );
			return false;	
		}
	 ...
	 * @see billets/admin/tables/BilletsTable#move($change, $where)
	 */
	...
	public function getRoot()
	{
		...
		if ( in_array( 'isroot', array_keys( $this->getProperties() ) ) ) 
		{
				...
				else 
			{
				...
				$this->setError( JText::_('COM_BILLETS_MULTIPLE_ROOTS_DEFINED') );
			}
		}
		...
		if (!$return)
		{
			...
			if (count($result) == 1) 
			{
				// there is only one record with parentid = 0
				$return = $result[0];
			}
				else
			{
				...
				{
					$this->setError( JText::_('COM_BILLETS_ROOT_NOT_FOUND_BY_UNIQUENESS') );
					return false;
				}
			}			
		}
	...
	public function orderUp($oid=null)
	{
		...
		if (empty($node->$key)) 
		{
			$this->setError( JText::_('COM_BILLETS_COULD_NOT_LOAD_NODE') );
			$this->_unlock();
			return false;
		}
		...
		$sibling = clone $this;
		$sibling->load( $node->lft - 1, 'rgt' );
		if (empty($sibling->$key)) 
		{
			// if one doesn't exist, #fail
			$this->setError( JText::_('COM_BILLETS_COULD_NOT_LOAD_SIBLING') );
			$this->_unlock();
			return false;
		}
