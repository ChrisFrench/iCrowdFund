 ...
 * @since		1.6
 */
class BilletsQueryElement extends JObject
{
 ...
 * @since		1.6
 */
class BilletsQuery extends JObject
{
	...
	public function select($columns)
	{
		...
		if (is_null($this->_select)) {
			$this->_select = new BilletsQueryElement('SELECT', $columns);
		} else {
	...
	public function from($tables)
	{
		if (is_null($this->_from)) {
			$this->_from = new BilletsQueryElement('FROM', $tables);
		} else {
	...
	public function join($type, $conditions)
	{
		...
		$this->_join[] = new BilletsQueryElement(strtoupper($type) . ' JOIN', $conditions);
	...
	public function where($conditions, $glue='AND')
	{
		if (is_null($this->_where)) {
			...
			$this->_where = new BilletsQueryElement('WHERE', $conditions, "\n\t$glue ");
		} else {
	...
	public function group($columns)
	{
		if (is_null($this->_group)) {
			$this->_group = new BilletsQueryElement('GROUP BY', $columns);
		} else {
	...
	public function having($conditions, $glue='AND')
	{
		if (is_null($this->_having)) {
			$glue = strtoupper($glue);
			$this->_having = new BilletsQueryElement('HAVING', $conditions, "\n\t$glue ");
		} else {
	...
	public function order($columns)
	{
		if (is_null($this->_order)) {
			$this->_order = new BilletsQueryElement('ORDER BY', $columns);
		} else {
