...
<h3><?php echo JText::_('COM_BILLETS_SELECT_MANAGERS_FOR'); ?>: <?php echo BilletsHelperCategory::getTitle( $row->id, 'flat' ); ?></h3>
    ...
    <table>
        <tr>
            ...
            <td nowrap="nowrap">
                ...
                <?php // BilletsSelect::category( @$state->filter_parentid, 'filter_parentid', $attribs, 'parentid', true, true ); ?>
            </td>
        </tr>
    </table>
	...
	<table class="adminlist" style="clear: both;">
        ...
        <?php foreach (@$items as $item) : ?>
            <tr class='row<?php echo $k; ?>'>
				...
				<td style="text-align: center;">
					<?php $already = BilletsHelperManager::isCategory( $item->id, $row->id, '1' ); ?>
					<?php $defined = BilletsHelperManager::isCategory( $item->id, $row->id, '1', 'view', '1' ); ?>
					<?php $user = JFactory::getUser( $item->id );
					if ($user->get('gid') == '25' ) 
					{
						echo "[".JText::_('COM_BILLETS_SUPER_ADMIN')."]<br/>";
					} 
						else
					{
						if (isset($already->id) && empty($defined)) { echo "[".JText::_('COM_BILLETS_INHERITED')."]<br/>"; }
					}
					?>
					<?php echo DSCGrid::enable(isset($already->id), $i, 'selected_'); ?>
				</td>
				<td style="text-align: center;">
					<?php $getsEmails = BilletsHelperManager::getsEmails( $item->id, $row->id ); ?>
